...
def add_high_importance_features(df: pd.DataFrame, cap: int) -> pd.DataFrame:
    ...
    df["yday_vol_log_ret"]   = np.log((v / v.shift(1)).replace(0, np.nan)).clip(-10, 10)
    df["yday_open_log_ret"]  = np.log((o / o.shift(1)).replace(0, np.nan)).clip(-10, 10)
    df["yday_close_log_ret"] = np.log((c / c.shift(1)).replace(0, np.nan)).clip(-10, 10)
    df["yday_high_log_ret"]  = np.log((h / h.shift(1)).replace(0, np.nan)).clip(-10, 10)
    df["yday_low_log_ret"]   = np.log((l / l.shift(1)).replace(0, np.nan)).clip(-10, 10)
    ...
    # Volume log returns
    df["vol_log_ret"]    = np.log((v / v.shift(1)).replace(0, np.nan)).clip(-10, 10)
    df["vol_log_ret_5d"] = np.log((v / v.shift(5)).replace(0, np.nan)).clip(-10, 10)

    # Momentum (log)
    for mn in [5, 10, 20]:
        if cap > mn:
            df[f"mom_log_{mn}d"] = np.log((c / c.shift(mn)).replace(0, np.nan)).clip(-10, 10)
